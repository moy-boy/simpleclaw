...
ACP_AGENT_LIST_RAW="${OPENCLAW_LIVE_ACP_BIND_AGENTS:-${OPENCLAW_LIVE_ACP_BIND_AGENT:-codex}}"
TEMP_DIRS=()
...
openclaw_live_acp_bind_resolve_auth_provider() {
  case "${1:-}" in
    codex) printf '%s\n' "codex-cli" ;;
    *)
      echo "Unsupported OPENCLAW_LIVE_ACP_BIND agent: ${1:-} (expected codex)" >&2
      return 1
      ;;
  esac
}
...
agent="${OPENCLAW_LIVE_ACP_BIND_AGENT:-codex}"
case "$agent" in
  codex)
...
if ((${#ACP_AGENTS[@]} == 0)); then
  echo "No ACP bind agents selected. Use OPENCLAW_LIVE_ACP_BIND_AGENTS=codex." >&2
  exit 1
fi
