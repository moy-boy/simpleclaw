...
describe("syncPluginVersions", () => {
  ...
  it("uses the base release version for beta changelog entries", () => {
    ...
    fs.mkdirSync(path.join(rootDir, "extensions/discord"), { recursive: true });
    fs.writeFileSync(
      path.join(rootDir, "extensions/discord/CHANGELOG.md"),
      "# Changelog\n\n## 2026.5.2\n\n### Changes\n\n- Previous release.\n",
      "utf8",
    );
    ...
    expect(summary.changelogged).toEqual(["@openclaw/discord"]);
