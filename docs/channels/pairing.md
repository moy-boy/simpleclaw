---
summary: "Pairing overview: approve who can DM you and which nodes can join"
read_when:
...
1. **DM pairing**: who is allowed to talk to the bot over Telegram or Discord.
2. **Node pairing**: which devices or nodes are allowed to join the gateway network.

Security context: [Security](/gateway/security)

## 1) DM pairing

When a supported channel is configured with DM policy `pairing`, unknown senders get a short code and their message is **not processed** until you approve.
...
Supported DM pairing channels are `telegram` and `discord`.
...
If no command owner is configured yet, approving a DM pairing code also bootstraps `commands.ownerAllowFrom` to the approved sender, such as `telegram:123456789` or `discord:123456789012345678`. That gives first-time setups an explicit owner for privileged commands and exec approval prompts. After an owner exists, later pairing approvals only grant DM access; they do not add more owners.

### Reusable sender groups

Use top-level `accessGroups` when the same trusted sender set should apply to Telegram and Discord, or to both DM and group allowlists.
...
```json5
{
  ...
  channels: {
    ...
    discord: { dmPolicy: "allowlist", allowFrom: ["accessGroup:operators"] },
  },
}
```
...
Treat these as sensitive because they gate access to your assistant.
...
2. The bot replies with two messages: an instruction message and a separate setup code message.
...
If the same device retries with different auth details, for example different role, scopes, or public key, the previous pending request is superseded and a new `requestId` is created.
...
- `pending.json` with short-lived pending requests
- `paired.json` with paired devices and tokens

### Notes

- The legacy `node.pair.*` API, exposed through `openclaw nodes pending|approve|reject|remove|rename`, is a separate gateway-owned pairing store. WS nodes still require device pairing.
