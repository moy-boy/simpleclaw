...
- Crestodian first-run Docker smoke: `pnpm test:docker:crestodian-first-run`
  - Starts from an empty OpenClaw state dir, verifies the modern onboard
    ...
    writes, validates config, and verifies audit entries.
...
- ACP bind smoke: `pnpm test:docker:live-acp-bind` (script: `scripts/test-live-acp-bind-docker.sh`; covers the Codex/OpenAI ACP bind path)
